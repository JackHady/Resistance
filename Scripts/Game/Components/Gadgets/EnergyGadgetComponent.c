class SCR_EnergyGadgetComponentClass : SCR_GadgetComponentClass
{
}

class SCR_EnergyGadgetComponent : SCR_GadgetComponent
{
	...
	protected const float DURATION = 1.0; // Duration of the drinking animation in seconds
	...
	protected void ApplyMetabolismEffect()
	{
    	...
    	if (characterOwner)
    	{
        	...
        	{
            	metabolismComponent.IncreaseEnergy(m_iEnergyValue); // Increase hydration
        	}
    	}
	}
	...
	protected void PlaySound(string soundName)
	{
    	...
    	if (soundComp)
    	{
        	...
        	GetGame().GetCallqueue().CallLater(soundComp.SoundEvent, 0, false, soundName);
    	}
	}
