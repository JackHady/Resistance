class JH_SCR_PlayerHUD : SCR_InfoDisplay
{
	//--------------------------- CONFIG VALUES ---------------------------
	...
	protected ImageWidget m_wStaminaOutline;
	protected ImageWidget m_wHealthOutline;
	protected ImageWidget m_wThirstOutline;
	protected ImageWidget m_wHungerOutline;
	...
	void OnHealthChange(float value)
	{
		...
		if (!m_wHealthOutline)
		{
			m_wHealthOutline = ImageWidget.Cast(m_wHealthIndicator.FindAnyWidget("m_healthOutline"));
			if (!m_wHealthOutline) return;
		}
		...
		else
		{
			...
			SetProgressColor(m_wHealthProgress, m_wHealthOutline, value);
		}
	}
	...
	void OnStaminaChange(float value)
	{
		...
		if (!m_wStaminaOutline)
		{
			m_wStaminaOutline = ImageWidget.Cast(m_wStaminaIndicator.FindAnyWidget("m_staminaOutline"));
			if (!m_wStaminaOutline) return;
		}
		...
		else
		{
			if (!float.AlmostEqual(m_fPreviousValues[1] , m_currentStam))
			{
				...
				if (!m_wStaminaProgressBar)
				{
					...
					SetProgressColor(m_wStaminaProgress, m_wStaminaOutline, value);//TODO: Make this only call every once and a while similar to the fade
				}
				else
				{
					m_wStaminaProgressBar.SetCurrent(value);
					SetProgressColor(m_wStaminaProgressBar, m_wStaminaOutline, value);//TODO: Make this only call every once and a while similar to the fade
				}
	...
	void OnThirstChange(float value)
	{
		...
		if (!m_wThirstOutline)
		{
			m_wThirstOutline = ImageWidget.Cast(m_wThirstIndicator.FindAnyWidget("m_thirstOutline"));
			if (!m_wThirstOutline) return;
		}
	...
	void OnHungerChange(float value)
	{
		...
		if (!m_wHungerOutline)
		{
			m_wHungerOutline = ImageWidget.Cast(m_wHungerIndicator.FindAnyWidget("m_hungerOutline"));
			if (!m_wHungerOutline) return;
		}
